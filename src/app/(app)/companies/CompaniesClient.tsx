            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: 7, fontSize: 13, color: 'var(--text)', outline: 'none',
                }}
              />
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {filtered.length === 0 ? (
              ...
              return (
                <button
                  key={company.id}
                  onClick={() => setSelected(isActive ? null : company.id)}
                  style={{
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
          {!selectedCompany ? (
            <div style={{
              ...
              <div style={{ fontSize: 12 }}>Click on a company to see its contacts</div>
            </div>
          ) : (
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>
                    ...
                    {selectedCompany.website && (
                      <> · <a href={selectedCompany.website} target="_blank" rel="noreferrer" style={{ color: 'var(--gold)', textDecoration: 'none' }}>{selectedCompany.website}</a></>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: 'var(--surface-1)', border: '1px solid var(--hairline)', borderRadius: 12, overflow: 'hidden' }}>
                ...
                {companyContacts.length === 0 ? (
                  ...
                  <div key={c.id} style={{
                    ...
                    borderBottom: i < companyContacts.length - 1 ? '1px solid var(--hairline)' : 'none',
                  }}>
                    <div style={{
                      width: 34, height: 34, borderRadius: 8,
                      background: 'var(--surface-3)', border: '1px solid var(--hairline)',
                      display: 'grid', placeItems: 'center',
                      fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', flex: '0 0 auto',
                    }}>
